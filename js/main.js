    ...
    Planet.addEventListener("click", () => {
      ...
      for (let s = 0; s < PlanetSections.length; s++) {
        ...
        if (Planet.classList.contains(Id)) {
          ...
          const PlanetsMenuItem = Planet.querySelectorAll('.right .planets li')
          const NewPlanet = document.querySelector('.planet:not(.hidden)')
          const NewPlanetMenu = NewPlanet.querySelectorAll('.right .planets li')
          for (const CurrentPlanetMenu of NewPlanetMenu) {
            if( CurrentPlanetMenu.getAttribute('rel') == Id) {
              CurrentPlanetMenu.classList.add('active')
            }
          }
        }
      }
